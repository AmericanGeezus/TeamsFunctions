  ...
  process {
    ...
    foreach ($Id in $Identity) {
      ...
      try {
        ...
        if ( $User ) {
          $VoiceRouting = $null
          $VoiceRouting = [TFVoiceRouting]::new()
          $VoiceRouting.Identity = $User.UserPrincipalName
          $VoiceRouting.TenantDialPlan = $User.TenantDialPlan
          $VoiceRouting.DialedNumber = $User.DialedNumber
          $VoiceRouting.OnlineVoiceRoutingPolicy = $User.OnlineVoiceRoutingPolicy
        }
        else {
          throw
        }
      ...
      if ($DialedNumber) {
        ...
        $VoiceRouting.EffectiveDialPlan = $EffectiveTDP.DialPlanName
        if ($PSBoundParameters.ContainsKey('Debug')) {
        ...
        if ( $EffectiveTDP.TranslatedNumber ) {
          ...
          $VoiceRouting.TranslatedNumber = $EffectiveTDP.TranslatedNumber
        }
      }
        ...
        if ($VoiceRoutes) {
          if ($DialedNumber) {
            ...
            if ( $MatchedVoiceRoutes ) {
              ...
              $VoiceRouting.OnlinePstnUsage = $MatchedVoiceRoutes[0].PSTNUsage

              # Populating MatchedVoiceRoutes
              $MatchedVoiceRoutesByPriority = $MatchedVoiceRoutes | Where-Object { $_.PSTNUsage -eq $VoiceRouting.OnlinePstnUsage } | Sort-Object Priority
              $VoiceRouting.MatchedVoiceRoutes = $MatchedVoiceRoutesByPriority.Name -join (', ')
              #if ( $MatchedVoiceRoutes.Count -gt 1 ) { #TEST This could be used to attach all with .ToString (like AA)  }
              Write-Verbose "OVP '$($User.OnlineVoiceRoutingPolicy)' - OPU '$($VoiceRouting.OnlinePstnUsage)' - Matching Voice Route(s): $($VoiceRouting.MatchedVoiceRoutes)"

              # Selecting Voice Route
              $UsedVoiceRoute = $null
              $UsedVoiceRoute = $MatchedVoiceRoutesByPriority | Select-Object -First 1

              $VoiceRouting.OnlineVoiceRoute = $UsedVoiceRoute.Name
              $VoiceRouting.MatchingPattern = $UsedVoiceRoute.MatchingPattern
              $VoiceRouting.Translation = $UsedVoiceRoute.Translation
              $VoiceRouting.OnlinePstnGateway = $($UsedVoiceRoute.OnlinePstnGatewayList -join (', '))
            ...
            # Selecting PSTN Usage of the first Voice Route
            $VoiceRouting.OnlinePstnUsage = $VoiceRoutes[0].PSTNUsage
