			...
			foreach ($ResourceAccount in $ResourceAccounts) {
				...
				try {
					...
					$AssocObject   = switch ($Assocociation.ConfigurationType) {
						"CallQueue"     { Get-CsCallQueue -Identity $A.ConfigurationId}
						"AutoAttendant" { Get-CsAutoAttendant -Identity $A.ConfigurationId}
					}
				}
				catch {
					$AssocObject	= $null
		...
		try {
			...
			foreach ($ResourceAccount in $ResourceAccounts) {
				...
				try {
					$Assocociation = Get-CsOnlineApplicationInstanceAssociation -Identity $ResourceAccount.ObjectId -ErrorAction SilentlyContinue
					$AssocObject   = switch ($Assocociation.ConfigurationType) {
						"CallQueue"     { Get-CsCallQueue -Identity $A.ConfigurationId -ErrorAction SilentlyContinue}
						"AutoAttendant" { Get-CsAutoAttendant -Identity $A.ConfigurationId -ErrorAction SilentlyContinue}
					}
				}
				catch {
					$AssocObject	= $null
